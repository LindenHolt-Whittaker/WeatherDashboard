...
export default function ForecastCard({ forecast, unit, index }: ForecastCardProps) {
  ...
  return (
    <div className="forecast-card">
      <div className="forecast-day">{getDayLabel(forecast.datetime, index)}</div>
      <WeatherIcon icon={forecast.icon} />
      <div className="forecast-conditions">{forecast.conditions}</div>
      <div className="forecast-temps">
        <span className="temp-high">{Math.round(maxTemp)}°{unit}</span>
        <span className="temp-low">{Math.round(minTemp)}°{unit}</span>
      </div>
    </div>
  );
}
